...
# Original design and primary implementation by @warrentc3: https://github.com/warrentc3
...
__all__ = [
    "convert_xmltv_offset",
    "convert_xmltv_naive_local",
    "convert_xmltv_naive_utc",
    "convert_xmltv_utc",
...
_SHAPE_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    ("xmltv-utc", re.compile(r"[0-9]{14} 0000")),
    (
        "py-doc-12h",
        re.compile(
...
def dt_to_stamp(value: datetime) -> str:
    ...
    try:
        ...
        raise ValueError("datetime is outside Python's UTC range") from exc
...
# Detector class: xmltv-naive
def convert_xmltv_naive_local(value: str) -> str:
    """Detector class: ``xmltv-naive``. Interpret its wall time as local."""
    return _from_naive_local(
        value,
        fmt="%Y%m%d%H%M%S",
        shape="YYYYMMDDHHMMSS",
        lexical_pattern=r"[0-9]{14}",
    )


def convert_xmltv_naive_utc(value: str) -> str:
    """Detector class: ``xmltv-naive``. Interpret its wall time as UTC."""
    return _from_naive_utc(
        value,
        fmt="%Y%m%d%H%M%S",
        shape="YYYYMMDDHHMMSS",
...
# Detector class: xmltv-offset
def convert_xmltv_offset(value: str) -> str:
    """Detector class: ``xmltv-offset``. Convert its timestamp shape."""
    return _from_explicit_offset(
        value,
        fmt="%Y%m%d%H%M%S %z",
...
# Detector class: xmltv-utc
def convert_xmltv_utc(value: str) -> str:
    """Detector class: ``xmltv-utc``. Convert its timestamp shape."""
    return _from_explicit_utc(
        value,
...
# Detector class: numeric-timestamp
def _parse_unix_time(value: str) -> datetime:
    ...
    if not _INT32_MIN <= seconds <= _INT32_MAX:
        raise ValueError("Unix time must fit in the signed 32-bit seconds range")
...
def convert_unix_time_utc(value: str) -> str:
    """Detector class: ``numeric-timestamp``. Convert standard Unix time."""
...
def convert_windows_filetime_utc(value: str) -> str:
    """Detector class: ``numeric-timestamp``. Convert standard FILETIME."""
