...
/* ---------- add ---------- */
function localDateTimeToStamp(value) {
...
$("add-form").addEventListener("submit", async (event) => {
  ...
  if (expiresRaw) payload.expires_at = localDateTimeToStamp(expiresRaw);
  try {
...
PAGE = (
    ...
    .replace(
        "__AMOS_RETENTION_MIN_HALF_LIVES__",
        format(RETENTION_MIN_HALF_LIVES, "g"),
    )
    .replace(
        "__AMOS_TOAST_DURATION_MILLISECONDS__",
        str(WEB_UI_TOAST_DURATION_MILLISECONDS),
    )
    .replace(
        "__AMOS_GRAPH_SETTLE_FRAME_THRESHOLD__",
        str(WEB_UI_GRAPH_SETTLE_FRAME_THRESHOLD),
    )
)
