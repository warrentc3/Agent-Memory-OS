...
| **Phase 2.3** | 動態上下文編排 (Dynamic Context Orchestration, DCO) | 🔵 ACTIVE | **目前重點**：實現主動的上下文快照與狀態恢復機制。 |
...
### 核心目標 (Goals)
- [ ] **`ContextSnapshot` 實作**：定義並實現快照結構，封裝目標、假設、關鍵變數及推理鏈。
- [ ] **API 開發**：
    - [ ] `offload_context`: 將當前狀態序列化並存儲至記憶庫。
    - [ ] `reload_context`: 恢復指定或最新的會話快照。
- [ ] **精度驗證**：確保狀態恢復後的上下文精度，無關鍵資訊丟失。
